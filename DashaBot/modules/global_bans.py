...
import DashaBot.modules.sql.global_bans_sql as sql
from DashaBot.modules.sql.users_sql import get_user_com_chats
...
from DashaBot.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if user_id == bot.id:
        message.reply_text("You want gban?")
        return
...
def ungban(update: Update, context: CallbackContext):
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, "ungban complete!")
    ...
    if ungban_time > 60:
        ...
        message.reply_text(f"Person has been ungbanned. Took {ungban_time} min")
    else:
        message.reply_text(f"Person has been ungbanned. Took {ungban_time} sec")
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    if not banned_users:
        update.effective_message.reply_text(
            "There's no gbanned users! You're kinder than I expected..."
        )
        return
...
def check_and_ban(update, user_id, should_message=True):
    ...
    if sw_ban:
        ...
        if should_message:
            update.effective_message.reply_text(
                f"This user ({sw_ban.id}) is globally banned by spamwatch!"
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if should_message:
            text = (
                f"This user ({user_id}) is globally banned!"
            )
            user = sql.get_gbanned_user(user_id)
            if user.reason:
                text += f"Ban Reason: {html.escape(user.reason)}"
