...
from DashaBot.modules.sql import antiflood_sql as sql
...
from DashaBot.modules.sql.approve_sql import is_approved
...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        msg.reply_text(
            "I can't restrict people here, give me permissions first! Until then, I'll disable antiflood."
        )
        sql.set_flood(chat.id, 0)
        return (
            "<b>{}:</b>"
            "\n#INFO"
            "\nDon't have enough permission to restrict users so automatically disabled antiflood".format(
                chat.title
            )
        )
...
def set_flood_mode(update, context):
    ...
    if args:
        if args[0].lower() == "ban":
            ...
            if len(args) == 1:
                teks = (
                    update.effective_message, "It looks like you tried to set time value for antiflood but you didn't specified time; Try, `/setfloodmode tmute <timevalue>`",
                )
                send_message(update.effective_message, teks, parse_mode="markdown")
                return
...
__help__ = """
*Command for Anyone:*
- `/flood`*:* Gets the current flood control settings

*Commands for Admins:*
- `/setflood <count/no/off>`*:* Enable or disable flood control
- `/setfloodmode <ban/kick/mute/tban/tmute>`*:* Action to perform when user exceeded the flood limit

*Note:*
- For tban and tmute flood control action you should specify the time (i.e 5m, 1h, 2d, 3w)
 """
